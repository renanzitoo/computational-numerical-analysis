...
void gaussJordanElimination(int n, float* a, float* b, int* colOrder) {
    for (int r = 0; r < n; r++) {
        ...
        float pivot = a[getOffset(r, r, n)];
        if (fabs(pivot) < 1e-10) {
            printf("Warning: Near-zero pivot detected at position (%d, %d)\n", r + 1, r + 1);
        }
        
        for (int j = 0; j < n; j++) {
            a[getOffset(r, j, n)] /= pivot;
        }
        b[r] /= pivot;
        
        for (int i = 0; i < n; i++) {
            if (i != r) {
                float m = a[getOffset(i, r, n)];
                for (int j = 0; j < n; j++) {
...
void reorderSolution(int n, float* b, int* colOrder, float* finalX) {
    for (int i = 0; i < n; i++) {
        finalX[colOrder[i]] = b[i];
    }
}
...
int main() {
    ...
    gaussJordanElimination(n, a, b, colOrder);
    reorderSolution(n, b, colOrder, finalX);
